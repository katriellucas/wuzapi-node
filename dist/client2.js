...
*/
var logger = {
	request: (0, import_browser.default)("wuzapi:request"),
	response: (0, import_browser.default)("wuzapi:response"),
	error: (0, import_browser.default)("wuzapi:error"),
	info: (0, import_browser.default)("wuzapi:info")
};
//#endregion
...
var WuzapiError = class extends Error {
	code;
	details;
...
var BaseClient = class {
	...
	constructor(config) {
		...
		this.axios.interceptors.response.use((response) => response, (error) => {
			if (error.response) {
				...
				throw new WuzapiError(body?.code || error.response.status, resolveErrorMessage(body, error.message), body);
			} else if (error.request) throw new WuzapiError(0, "Network error: No response from server");
			else throw new WuzapiError(0, error.message);
		});
	}
	/**
	...
	*/
	buildHeaders(options) {
		...
		if (!token) throw new WuzapiError(401, isAdmin ? "No admin token provided. Set `adminToken` in the client config, or pass `{ token }` in the request options." : "No user token provided. Set `token` in the client config, or pass `{ token }` in the request options.");
		return {
			...this.defaultHeaders,
	...
	async request(method, endpoint, data, options) {
		...
		if (this.config.debug) logger.request(`[${method}] ${endpoint}`, {
			headers,
			data
		});
		...
		if (this.config.debug) logger.response(`[${method}] ${endpoint}`, {
			status: response.status,
			data: response.data
		});
		const { code, data: payload, success } = response.data;
		if (!success || code < 200 || code >= 300) throw new WuzapiError(code, resolveErrorMessage(response.data, "API request failed"), response.data);
		return payload;
	}
...
Object.defineProperty(exports, "WuzapiError", {
	enumerable: true,
	get: function() {
		return WuzapiError;
	}
