...
//#region src/modules/chat.ts
var ChatModule = class extends require_client.BaseClient {
	/**
	* Send a text message
	*/
	...
	*/
	async sendTemplate(request, options) {
		return this.post("/chat/send/template", request, options);
	}
	/**
	...
	*/
	async sendPoll(groupJID, header, options, id, requestOptions) {
		...
		return this.post("/chat/send/poll", request, requestOptions);
	}
	/**
	* Edit a message
	*/
	...
	*/
	async getChatHistory(chatJid, limit, options) {
		const queryParams = [`chat_jid=${encodeURIComponent(chatJid)}`];
		if (limit !== void 0) queryParams.push(`limit=${limit}`);
		return this.get(`/chat/history?${queryParams.join("&")}`, options);
	}
	/**
	...
	*/
	async requestUnavailableMessage(chat, sender, messageId, options) {
		...
		return this.post("/chat/request-unavailable-message", request, options);
	}
	/**
	...
	*/
	async downloadSticker(request, options) {
		return this.post("/chat/downloadsticker", request, options);
	}
};
//#endregion
